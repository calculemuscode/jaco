...
export class PickyLexer {
    private typeIds = Set<string>();
    addIdentifiers(x: string) {
        this.typeIds = this.typeIds.add(x);
    }
    resetIdentifiers() {
        this.typeIds = Set<string>();
    }
...
export const pickyLexer = new PickyLexer();
...
export const util = {
    ...
    BinaryExpression: ([left, s1, opertoks, s2, right]: [
        ...
        Token,
        ast.Expression
    ]): ast.BinaryExpression | ast.LogicalExpression | ast.AssignmentExpression => {
